...
describe('test /v2/user/:uid/lock', function() {
  ...
  it('should / status 401 when the not sigin', async function() {
    try {
      const res = await request.patch(`/v2/user/${mockUser.id}/lock`).expect(401);
  ...
  it('should / status 401 when the no permission', async function() {
    try {
      ...
      res = await request.patch(`/v2/user/${mockUser.id}/lock`).set('Authorization', res.text).expect(401);
  ...
  it('should / status 403 when the user is you', async function() {
    try {
      ...
      res = await request.patch(`/v2/user/${mockUser2.id}/lock`).set('Authorization', res.text).expect(403);
  ...
  it('should / status 403 when the user role more than yours', async function() {
    try {
      ...
      res = await request.patch(`/v2/user/${mockUser3.id}/lock`).set('Authorization', res.text).expect(403);
  ...
  it('should / status 200 when the action is lock', async function() {
    try {
      ...
      res = await request.patch(`/v2/user/${mockUser.id}/lock`).set('Authorization', res.text).expect(200);
