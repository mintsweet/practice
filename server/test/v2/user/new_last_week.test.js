...
describe('test /v2/users/new_last_week', function() {
  let mockUser;
  let mockUser2;
  ...
  it('should / status 401 when the not signin', async function() {
    try {
      await request.get('/v2/users/new_last_week').expect(401);
    } catch(err) {
  ...
  it('should / status 401 when the no permission', async function() {
    ...
    await request.get('/v2/users/new_last_week').set('Authorization', res.text).expect(401);
  });
  ...
  it('should / status 200', async function() {
    try {
      ...
      res = await request.get('/v2/users/new_last_week').set('Authorization', res.text).expect(200);
