...
describe('test /v2/users/list', async function() {
  let mockUser;
  let mockUser2;
  ...
  it('should / status 401 when the not signin', async function() {
    try {
      await request.get('/v2/users/list').expect(401);
    } catch(err) {
  ...
  it('should / status 401 when the no permission', async function() {
    try {
      ...
      await request.get('/v2/users/list').set('Authorization', res.text).expect(401);
    } catch(err) {
      should.ifError(err.message);
    }
  });
  ...
  it('should / status 200', async function() {
    try {
      ...
      res = await request.get('/v2/users/list').set('Authorization', res.text).expect(200);
