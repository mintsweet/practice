...
describe('test /v2/topic/:tid/lock', function() {
  ...
  it('should / status 401 when the not sigin', async function() {
    try {
      const res = await request.patch(`/v2/topic/${mockTopic.id}/lock`).expect(401);
  ...
  it('should / status 401 when the no permission', async function() {
    try {
      ...
      res = await request.patch(`/v2/topic/${mockTopic.id}/lock`).set('Authorization', res.text).expect(401);
  ...
  it('should / status 200 when the action is lock', async function() {
    try {
      ...
      res = await request.patch(`/v2/topic/${mockTopic.id}/lock`).set('Authorization', res.text).expect(200);
