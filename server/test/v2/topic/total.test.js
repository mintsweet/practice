...
describe('test /v2/user/total', function() {
  ...
  it('should / status 401 when the not signin', async function() {
    try {
      await request.get('/v2/topics/total').expect(401);
    } catch(err) {
  ...
  it('should / status 401 when the no permission', async function() {
    try {
      ...
      await request.get('/v2/topics/total').set('Authorization', res.text).expect(401);
    } catch(err) {
      should.ifError(err.message);
    }
  });
  ...
  it('should / status 200', async function() {
    try {
      ...
      res = await request.get('/v2/topics/total').set('Authorization', res.text).expect(200);
