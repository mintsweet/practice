...
router.get('/setting', Auth.userRequired, wrap(Render.setting)); // 设置页
router.get('/update_pass', Auth.userRequired, wrap(Render.updatePass)); // 修改密码页
router.get('/users/top100', wrap(Render.userTop100)); // 积分榜前一百页
router.get('/user/:uid', wrap(Render.userInfo)); // 用户信息页
router.get('/user/:uid/topic', wrap(Render.userTopic)); // 用户主题页
router.get('/user/:uid/star', wrap(Render.userStar)); // 用户点赞页
...
router.get('/topic/create', Auth.userRequired, wrap(Render.topicCreate)); // 创建话题页
router.get('/topic/update', Auth.userRequired, wrap(Render.topicUpdate)); // 更新话题页
router.get('/topic/:tid', wrap(Render.topicDetail)); // 话题详情页
router.get('/topics/search', wrap(Render.topicSearch)); // 话题搜索页
router.get('/notice/user', Auth.userRequired, wrap(Render.userNotice)); // 用户消息页
router.get('/notice/system', Auth.userRequired, wrap(Render.systemNotice)); // 系统消息页
...
router.post('/topic/create', Auth.userRequired, wrap(Topic.createTopic)); // 创建话题
router.delete('/topic/:tid', Auth.userRequired, wrap(Topic.deleteTopic)); // 删除话题
router.put('/topic/:tid', Auth.userRequired, wrap(Topic.editTopic)); // 更新话题
router.post('/topic/:tid/star_or_un', wrap(Topic.likeOrUn)); // 点赞或取消点赞话题
...
router.post('/reply/:rid/delete', wrap(Topic.deleteReply)); // 删除回复
router.post('/reply/:rid/edit', wrap(Topic.editReply)); // 编辑回复
