...
router.get('/forget-pass', wrap(Render.forgetPass)); // 忘记密码页
router.get('/setting', auth(), wrap(Render.setting)); // 设置页
router.get('/update-pass', auth(), wrap(Render.updatePass)); // 修改密码页
...
router.post('/forget-pass', wrap(User.forgetPass)); // 忘记密码
router.post('/setting', auth(), wrap(User.setting)); // 更新设置
router.post('/update-pass', auth(), wrap(User.updatePass)); // 修改密码
...
router.post('/topic/:tid/collect', wrap(Topic.collectOrUn)); // 收藏或取消收藏话题
