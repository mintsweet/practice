...
class Notice {
  ...
  async normalNotice(item) {
    ...
    switch (item.type) {
      case 'like':
        data.author = await UserProxy.getById(item.author_id, 'id nickname avatar');
        data.topic = await TopicProxy.getById(item.topic_id, 'id title');
        data.typeName = '喜欢了';
        break;
      case 'collect':
        data.author = await UserProxy.getById(item.author_id, 'id nickname avatar');
      ...
      case 'at':
        data.author = await UserProxy.getById(item.author_id, 'id nickname avatar');
        data.topic = await TopicProxy.getById(item.topic_id, 'id title');
        data.reply = await ReplyProxy.getById(item.reply_id, 'id content');
        data.typeName = '@了';
        break;
      case 'up':
        data.author = await UserProxy.getById(item.author_id, 'id nickname avatar');
  ...
  async getUserNotice(ctx) {
    ...
    const notices = await NoticeProxy.get(query, '', option);
