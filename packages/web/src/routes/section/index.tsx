...
import { Avatar, Button, Pagination, Plus } from '@mints/ui';
import { useParams, Link } from 'react-router';

import API from '@/api';
import { useAuth } from '@/auth-context';
import { TopicList } from '@/routes/home/topic-list';
...
export function SectionTopics() {
  ...
  const { loading, data } = useRequest(async () => {
    ...
    const [sections, topics] = await Promise.all([
      API.section.query(),
      API.topic.query({ page, sectionId }),
    ]);
  ...
  if (loading || !data || !data.currentSection) {
    return (
  ...
  const { sections, currentSection, topics } = data;
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        ...
        <div className="flex gap-6">
            ...
            <div className="sticky top-20 space-y-4">
              {user ? (
                <>
                  ...
                  <Link
                    to="/topic/create"
                    className="flex items-center gap-2 bg-zinc-900 hover:bg-zinc-800 text-white rounded-xl p-4 text-center font-medium text-sm shadow-sm hover:shadow-md transition-all"
                  >
                    <Plus size={20} />
                    Create Topic
                  </Link>
          ...
          <div className="flex-1 min-w-0">
            ...
            <Pagination className="mt-6" current={page} total={topics.total} />
          </div>
        </div>
      </div>
    </div>
